...
test.describe.serial("private finance app", () => {
  ...
  test("creates and closes a monthly snapshot through the wizard", async ({ page }) => {
    ...
    await expect(page.getByText("Posicao adicionada ao rascunho.")).toBeVisible();

    await page.locator('button[title="Remover posicao"]').last().click();
    await expect(page.getByText("Posicao removida do rascunho.")).toBeVisible();
    ...
    await expect(page.getByText("Saldos salvos e previa recalculada.")).toBeVisible();

    await page.getByRole("button", { name: "Fechar mes" }).click();
    await expect(page.getByText("Mes fechado com sucesso.")).toBeVisible();
  });
